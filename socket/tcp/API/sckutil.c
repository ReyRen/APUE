...
int connect_timeout(int fd, struct sockaddr_in *addr, unsigned int wait_seconds)
{
	...
	ret = connect(fd, (struct sockaddr*)addr, addrlen);//如果网络好，马上建立连接，
	if (ret < 0 && errno == EINPROGRESS)//那么就根据返回值表示客户和服务器端正在建立连接我们等一等，然后不能干等，所以自己进行控制
	{
		...
		else if (ret == 1)
		{
			...
			int sockoptret = getsockopt(fd, SOL_SOCKET, SO_ERROR, &err, &socklen);//当select监控到链接可读，并不能代表链接可用
							/*
								造成可读有几种结果：
									真正的链接起来
									建立的时候失败了，但是失败会回写失败信息
							*/
			if (sockoptret == -1)
			{
				return -1;
