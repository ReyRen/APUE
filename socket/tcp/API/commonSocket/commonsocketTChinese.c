		...
		unsigned int wait_seconds)
{
	...
	if (wait_seconds > 0)
		activate_nonblock(fd);//这里为什么设置非阻塞，
				/*
					因为TCPIP的在客户端连接服务器的时候，如果异常，connect(默认是阻塞额情况下)，返回时间是1.5RTT大约是75s以上
					贼恐怖
				*/
	//链接
	/*
	...
	if (ret < 0 && errno == EINPROGRESS)
	{
		...
		{
			/*
			一旦建立连接，select就开始监控一个是stdin（从键盘读），另一个是socketfd(从socket读)的两个读
			按理说最完整的情况是判断一把是哪个读，然后分别做操作，但这里就只缩了一个的监控
			*/
