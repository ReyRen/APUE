...
int main01()
{
	...
	//客服端链接
	if (connect(sockfd, (struct sockaddr*) (&srvaddr), sizeof(srvaddr)) < 0)//要把具体的TCP/IP协议结构体转换成通用的
	{
		perror("fun socket\n");
		exit(0);
	}
	...
	while (fgets(sendbuf, sizeof(sendbuf), stdin) != NULL)//这种就是长连接，不断
	{
		//向服务写数据
	...
	close(sockfd);//这是关闭了自己的，虽然和服务器新常见的socket的fd一样
	return 0;
}
