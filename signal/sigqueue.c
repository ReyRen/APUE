...
//用新的回到函数接受数据。。。。  
void myHandle_forsigaction(int signum, siginfo_t *s_t, void *p)//s_t这个是linux内核创了个结构体，并且给分配好内存然后将值传进去了
{
	//printf("recv a sig=%d data=%d data=%d\n", sig, info->si_value.sival_int, info->si_int);
		int myint = 0;
		printf("recv signum : %d \n", signum);
		myint = s_t->si_value.sival_int;//siginfo_t结构体中有个si_value域，这个域又是个结构体sigval_t， 也就是union sigval {int   sival_int; void *sival_ptr;}
		printf("%d %d \n", myint, s_t->si_int );
...
int main(int argc, char *argv[])
{
	...
	if (pid == 0)
	{
	   ...
	   mysigval.sival_int = 222;	
		...
		for (i=0; i<10; i++)
		{
			ret  = sigqueue(getppid(), SIGINT, mysigval);//向父进程中发送额外的信息然后父进程中就能通过sigaction收到额外的数据了
			if (ret != 0)
			{
				printf("sigqueue .....\n");
				exit(0);
