...
=======
		/* get the first line of request like: GET / */
		if((request_line = strtok(buf, CRLF)) == NULL){
...
>>>>>>> d9e1aecf356fee9cc480c282832da9e4b2d622a6
		...
		/* got the second line of request, like: last modified since */
		if((conditional_request_line = strtok(NULL, CRLF)) == NULL){
			perror("Cannot handle request");	
		}
...
=======
					since_time_length = strlen(conditional_request_line) - if_modified_since - white_space + 1;//this is the real time length
					/* allocate memory that is automatically freed */
					if((real_date = (char *)alloca(since_time_length)) == NULL){
						perror("Cannot alloc memory");
					}
					strncpy(real_date, conditional_request_line + if_modified_since + white_space, since_time_length - 1);//copy to real_data mem
>>>>>>> d9e1aecf356fee9cc480c282832da9e4b2d622a6
		...
		else if(strcasecmp(token[0], "HEAD") == 0)
		{
			...
			if(strcpy(requester.path, token[1]) == NULL){
				perror("Cannot handle the request");
			}
			...
			if(http_status == RESPONSE_STATUS_OK)
			{
				if(strncpy(requester.path, realpath_str, PATH_MAX + 1) == NULL){
					perror("Cannot handle the request");
				}
			}
		}
		...
		*/
		...
		(void)snprintf(logger.request_status, sizeof(logger.request_status), "%d", responser.response_code);
		(void)snprintf(logger.response_size, sizeof(logger.response_size), "%d", responser.content_length);
