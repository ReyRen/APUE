...
int isDir(const char * dir)
{
        ...
        if (dir == NULL)
        {   
                printf("You provided directory is null\n");
                return 1;
        }   
        if(stat(dir, &st) < 0)
        {   
        ...
        {   
                printf("%s you provided is not a directory\n", dir);
                return 1;
        }    
...
void waiting_request(int client_socket)
{
	...
	do
	{
		...
		ret = select(client_socket + 1, &request_read_fd, NULL, NULL, &tv);
...
int resp_header(struct response * responser, int client_socket)
{
	...
	if(responser->last_modified != -1)
	{
		...
		if(write_length < 0)
		{
			printf("write last modified time fail\n");	
			return -1;
		}
		buf_size -= write_length;
		bufP += write_length;
	...
	if(strlen(responser->content_type) > 0)
	{
		...
		if(write_length < 0)
		{
			printf("write content type fail\n");
			return -1;
		}
		buf_size -= write_length;
		bufP += write_length;
	...
	if(responser->content_length >= 0)
	{
		...
		if(write_length < 0)
		{
			printf("write content length fail\n");
			return -1;
		}
		buf_size -= write_length;
		bufP += write_length;
...
int parseuri(struct request * request, int * uri_status, struct flags * flag, char * realpath_str, int * cgi_request)
{
	...
	if(request->path[0] == '/' && request->path[1] == '~')
	{
		...
		//malloc
		username = (char *)malloc(i + 1);// 0
			
		strncpy(username, userdir, i);
		username[i] = '\0';//end
	...
	}else if(strstr(request->path, "/cgi-bin") == request->path && flag->c_dir != NULL)
	{
		...
		*cgi_request = 1;
		...
		strncpy(uri_path, server_real_path, strlen(server_real_path) + 1);
		...
		if(*chp == '?')
		{
			//interrupted 
			*chp = '\0';
			strncpy(request->query_string, ++chp, sizeof(request->query_string) - strlen("QUERY_STRING="));
		}
	}
	else
	{
	...
	if(strstr(uri_real_path, server_real_path) != NULL)//check again, if the dir append ok
	{
		*uri_status = RESPONSE_STATUS_OK;
		if(*cgi_request)
		{
			strncpy(realpath_str, (char*) uri_real_path, sizeof(uri_real_path));
		}else
		{
...
int check_index_html(const char * path, char * index_html)
{
	...
	if(strlen(path) + strlen(INDEX_HTML) < PATH_MAX)
	{
		...
		{
			strncat(index_html, INDEX_HTML, 11);
		}else
		{
			strncat(index_html, "/" INDEX_HTML, 12);
		}
	}
	if(stat(index_html, &st) == 0)
	{
		...
		{
			strncpy(index_html, path, PATH_MAX + 1);//directory named index.html
			return 0;
		}
				...
				int socket, struct response * responser)
{
	...
	if(requester->method == REQUEST_METHOD_GET || requester->method == REQUEST_METHOD_HEAD)
	{
		...
		{
			sprintf(query_env, "QUERY_STRING=%s", requester->query_string);
		}
